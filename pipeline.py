...
class AnimationPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def __call__(
        ...
        #                                                      mode='write', batch_size=context_batch_size)
        ...
        if isinstance(source_image, str):
            ...
            source_image = np.stack(source_image, 0)
            ref_image_latents = self.images2latents(source_image, latents_dtype).cuda()
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank != 0)):
            ...
            control_emb = self.controlnet(controlnet_cond_images)

            end_noise_pred = noise_pred
            for cur_ref_lants in ref_image_latents:
                appearance_encoder(
                    cur_ref_lants.unsqueeze(0).repeat(batch_size * (2 if do_classifier_free_guidance else 1), 1, 1, 1),
                    t,
    ...
    #                 # method_obj = getattr(self.unet,'forward')
